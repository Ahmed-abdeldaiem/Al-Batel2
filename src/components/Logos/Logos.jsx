  ...
  <div className="lg:fixed relative lg:start-0 lg:top-16 z-10 flex  flex-wrap  lg:flex-col w-full  lg:w-1/12 justify-center">
    {logos.map((logo, index)=>{
      ...
      <div key={index} className='p-2 w-3/12 md:w-2/12 lg:w-full '>
          ...
          <h4 className='text-[#3a497b] text-sm md:text-lg font-semibold border-2 text-center bg-slate-100/70 rounded-tl-2xl rounded-br-2xl border-[#3a497b] cursor-pointer transition-all duration-500 ease-linear hover:bg-slate-400/80 hover:shadow-lg '>{logo.word}</h4>
      </div>
      )
    })}
</div>
